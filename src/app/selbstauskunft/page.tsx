...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("selbstauskunft");
  return {
    title: t("meta.title"),
    description: t("meta.description"),
    alternates: alternates("/selbstauskunft"),
  };
}

export default async function SelbstauskunftLanding() {
  const t = await getTranslations("selbstauskunft");

  const start = `/selbstauskunft/start`;
    ...
    <main className="min-h-screen">
      ...
      <section className="mx-auto max-w-4xl px-6 pt-20 pb-12 text-center space-y-6">
        ...
        >
          {t.rich("hero.title", {
            mark: (chunks) => (
              <span className="rounded-md bg-[#6c5ce7]/15 px-1.5 text-[#6c5ce7]">
                {chunks}
              </span>
            ),
          })}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
          <Link
            href={start}
            className="inline-flex items-center gap-2 bg-[#6c5ce7] hover:bg-[#5b4bd6] text-white font-semibold px-5 py-2.5 rounded-lg transition-colors"
          >
            {t("hero.cta")}
        ...
        <p className="inline-flex max-w-full items-center gap-2 mx-auto text-center text-sm font-medium text-[#6c5ce7] bg-[#6c5ce7]/10 border border-[#6c5ce7]/20 rounded-full px-4 py-1.5">
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 space-y-12">
        ...
        <div className="grid sm:grid-cols-3 gap-6 lg:gap-8">
          {steps.map(({ icon: Icon, title, desc }, i) => (
            ...
            >
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#6c5ce7]/10 border border-[#6c5ce7]/20 text-[#6c5ce7]">
      ...
      <section className="mx-auto max-w-4xl px-6 py-20">
        <div className="rounded-2xl border border-[#6c5ce7]/20 bg-[#6c5ce7]/5 px-6 py-12 text-center space-y-5">
          ...
          <Link
            href={start}
            className="inline-flex items-center gap-2 bg-[#6c5ce7] hover:bg-[#5b4bd6] text-white font-semibold px-6 py-3 rounded-lg transition-colors"
          >
            {t("cta.button")}
            <ArrowRight className="h-4 w-4" />
          </Link>
